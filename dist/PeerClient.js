...
export class PeerClient {
    ...
    async defaultHandle(msg) {
        ...
        if (msg.kind === 'task.cancel') {
            await Promise.all(Array.from(this.driversById.values()).map((driver) => driver.cancel(msg.run_id).catch(() => undefined)));
            return;
        }
    ...
    async resolveAttention(message) {
        ...
        const candidates = message.driver
            ? [this.driversById.get(message.driver)].filter((driver) => Boolean(driver))
            : Array.from(this.driversById.values()).filter((driver) => typeof driver.resolveAttention === 'function');
        const driver = candidates.length === 1 && candidates[0]?.resolveAttention
            ? candidates[0]
            : null;
    ...
    async executeDispatch(msg) {
        ...
        if (isV2TaskDispatch(msg)) {
            try {
                ...
                if (msg.execution_envelope.runId !== msg.run_id ||
...
//# sourceMappingURL=PeerClient.js.map