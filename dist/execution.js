...
export function validateExecutionEnvelope(envelope) {
    ...
    if ((envelope.workRef.workstreamId ||
...
export function validateExecutionResult(result, envelope) {
    if (result.runId !== envelope.runId ||
    ...
    if (['technically_complete', 'outcome_pending', 'accepted'].includes(result.disposition) &&
...
export async function buildExecutionFinalizationRequest(draft, envelope) {
    const request = {
        ...draft,
...
export async function validateExecutionFinalizationRequest(request, envelope) {
    if (request.schemaVersion !== '1.0.0') {
        throw new Error('execution finalization schema version unsupported');
    }
    if (request.runId !== envelope.runId ||
...
export async function validateExecutionFinalizationResponse(response, request, envelope) {
...
export async function computeContractDigest(value) {
...
export async function verifyContractDigest(value, expectedDigest) {
